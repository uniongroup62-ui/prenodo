...
// (public_customer_accounts password reset via emailed link — NOT the
// gestionale flow).
//
...
// (eyebrow, heading, lead, reset form with its exact field names + the disabled
// email field and hidden email/token inputs, submit button), plus the
...
//
// The page reads ?token= and ?email= (plus tenant/next/return/location_id) from
// the URL, mirroring the PHP `$resetToken`/`$resetEmail` query params. The reset
// form is wired to the existing /api/account endpoint via fetch (JSON,
// action:"reset"). The legacy hidden _csrf field is dropped. On success the API
// starts a customer session, so we redirect to the `return` target like the PHP
// account_after_auth flow.
...
export function AccountResetFaithful() {
  const [error, setError] = useState("");
  const [busy, setBusy] = useState(false);
  // The PHP page carries the post-auth destination through `next`/`return`
  // query params, plus the reset `email`/`token` carried by the emailed link.
  ...
  // Build the destination once the reset succeeds, mirroring the PHP `return`
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/account", {
        ...
        body: JSON.stringify({ action: "reset", email: resetEmail, token: resetToken, password }),
      });
      const result = (await response.json()) as AccountResponse;
      if (!result.ok) {
        setError(result.error || "Reset non valido.");
        return;
      }
        ...
        <main className="account-main account-main--auth-flow">
          <div className="auth-stack">
            ...
            <section className="auth-card">
              ...
              <p className="lead">Scegli una nuova password per il tuo account cliente.</p>
              ...
              <form className="form" method="post" action="/api/account" onSubmit={handleSubmit}>
                ...
                <label>
                  Email <input type="email" value={resetEmail} disabled />
                </label>
