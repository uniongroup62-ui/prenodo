...
// (public_customer_accounts password recovery — NOT the gestionale flow).
//
...
// topbar/brand, the auth card (eyebrow, heading, lead, forgot form with its
// exact field names, submit button) and the "Torna al login" link, plus the
...
//
// The forgot form is wired to the existing /api/account endpoint via fetch
// (JSON, action:"forgot") instead of the legacy form POST. The legacy hidden
// _csrf field is dropped. Like the PHP page (which keeps $mode='forgot' and
// shows a success notice), on success we stay on the page and render the
// returned message in the original `.alert.alert-success` markup.
...
export function AccountForgotFaithful() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!result.ok) {
        setError(result.error || "Impossibile inviare il reset.");
        return;
      }
      // PHP stays on the forgot page and shows the returned success message.
      setMessage(result.message || "Se l'email e registrata, riceverai un link per reimpostare la password.");
    } catch {
        ...
        <main className="account-main account-main--auth-flow">
          <div className="auth-stack">
            ...
            <section className="auth-card">
              ...
              <p className="lead">Ti invieremo un link sicuro per reimpostare la password.</p>
