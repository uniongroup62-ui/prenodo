...
// (public_customer_accounts email-code verification step — NOT the gestionale
// flow).
//
// Markup is reproduced VERBATIM from the captured PHP output, keeping the
// original class names: the marketplace account topbar/brand, the auth card
// (eyebrow, heading, lead with the pending email interpolated exactly like the
// PHP `$pendingEmail` fallback, the code form with its `code-input` field, the
// separate resend form, and the "Torna al login" link), plus the right-hand
// visual card. Styling comes from /assets/css/app.css and
...
//
// The PHP page resolves the account from the pending-account session; here the
// account context (account_id + email) is carried on the URL query by the
// login / register next-step flow. The code form is wired to /api/account
// (JSON, action:"verify" with account_id + code) and the resend form to
// action:"resend_verification". The legacy hidden _csrf field is dropped. On a
// successful verify the API starts a customer session, so we redirect to the
// `return` target like the PHP account_after_auth flow.
...
export function AccountVerifyFaithful() {
  const [error, setError] = useState("");
  const [message, setMessage] = useState("");
  const [busy, setBusy] = useState(false);
  // The PHP page carries the post-auth destination through `next`/`return`
  // query params. The account context (id + pending email) is carried on the
  // URL by the login / register next-step flow that routes here.
  ...
  useEffect(() => {
    ...
    setAccountId(params.get("account_id") ?? "");
    setPendingEmail((params.get("email") ?? "").trim());
  }, []);
  ...
  async function handleVerify(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/account", {
        ...
        body: JSON.stringify({ action: "verify", account_id: accountId, code }),
      });
      ...
      // PHP starts a session and routes to the post-auth target.
  ...
  async function handleResend(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/account", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ action: "resend_verification", account_id: accountId }),
      });
      const result = (await response.json()) as AccountResponse;
      if (!result.ok) {
        setError(result.error || "Impossibile inviare il codice.");
        return;
      }
      setMessage("Codice reinviato via email.");
    } catch {
      setError("Operazione non riuscita. Riprova.");
    } finally {
      setBusy(false);
  ...
  // Preserve the PHP query string (next/return) on the "Torna al login" link.
  const linkQuery = (() => {
        ...
        <main className="account-main account-main--auth-flow">
          <div className="auth-stack">
            ...
            <section className="auth-card">
              ...
              <h1>Codice email</h1>
              <p className="lead">
                Inserisci il codice di 6 cifre inviato a {pendingEmail !== "" ? pendingEmail : "la tua email"}.
              </p>

              {error ? <div className="alert">{error}</div> : null}
              {message ? <div className="alert alert-success">{message}</div> : null}

              <form className="form" method="post" action="/api/account" onSubmit={handleVerify}>
              ...
              <form className="auth-resend-form" method="post" action="/api/account" onSubmit={handleResend}>
                ...
                <input type="hidden" name="location_id" value={locationId} />
                <input type="hidden" name="account_id" value={accountId} />
                <button className="btn auth-resend-button" type="submit" disabled={busy}>
                  Reinvia codice
                </button>
              </form>
              <div className="links">
                <a href={`/account/login${linkQuery}`}>Torna al login</a>
              </div>
            </section>
          </div>
